...
% and to the laser scans.  You can watch the movie "q2_soln.mp4" to 
% see what the results should look like.  The plot "q2_soln.png" shows
    ...
    for n=1:nparticles
        ...
        for beam=1:2         
            ...
            for p=0.45:(ogres/2):y_laser_max
                if (oglo(round(y_coord_prediction), round(x_coord_prediction)) >= 0)
                    continue
                end
                
                x_coord_prediction =  x_coord_prediction + p * cos(theta_prediction);
                if (x_coord_prediction > 300)
                    x_coord_prediction = 300;
                else
                    if (x_coord_prediction < 1)
                ...
                y_coord_prediction =  y_coord_prediction + p * sin(theta_prediction);
                if (y_coord_prediction > 180)
                    y_coord_prediction = 180;
                else
                    if (y_coord_prediction < 1)
            ...
            error = norm([x_coord_prediction; y_coord_prediction] -  [x_coord_laser; y_coord_laser]);
            gaussian_update = (1/sqrt(2 * pi * laser_var)) * exp(-error^2/(2 * laser_var));
