...
%
% requires: basic Matlab, 'gazebo_data.mat'
%
% S. Waslander, 2023
%
clear all;
...
% below.  See the plot 'q1_soln.png' for what your results should look
% like.
...
% noise.  See the plot 'q2_soln.png' for what your results should look
% like.
...
% truth look as crisp as the one in 'q3_soln.png'.  The first patch is
...
% ==========================
% Recall: Documentation of the dataset
% ==========================
%    ground truth poses: t_true x_true y_true theta_true
%    odometry measurements: t_odom v_odom omega_odom
%    laser scans: t_laser y_laser
%    - y_laser: has size=(989,640), where each time_stamp records 640 distance data
%                    from phi_min_laser to phi_max_laser
%    laser range limits: r_min_laser r_max_laser
%    laser angle limits: phi_min_laser phi_max_laser
...
y_laser = y_laser - 0.1;  % uncomment to apply Patch#2
for n=1:2
    if n==1
    ...
    for i=1:size(t_laser,1)
        ...
        %if true  % No Patch#1 applied
        if omega_interp(i) < 0.1  % Apply Patch#1
            T = [cos(theta_interp(i)), -sin(theta_interp(i)), 0, x_interp(i);    % reference to robot transformation in matrix SE(3)
                    sin(theta_interp(i)),  cos(theta_interp(i)), 0, y_interp(i);
                    0,                               0,                                1, 0;
                    0,                               0,                                0, 1];
            
            x_interp_robot = y_laser(i,:) .* cos_angles;
            y_interp_robot = y_laser(i,:) .* sin_angles;
            
            coord_robot = vertcat(x_interp_robot, ...
                                                  y_interp_robot, ... 
                                                  zeros(size(x_interp_robot)), ... 
                                                  ones(size(x_interp_robot)));  % 4 x size(y_laser(i))
            
            coord_reference = T * coord_robot; 
            if n==1
                scatter(coord_reference(1,:), coord_reference(2,:), 2, 'r');
            else
                scatter(coord_reference(1,:), coord_reference(2,:), 2, 'b');
            end
        end
...
axis equal;
print -dpng q3_w_patch1and2.png
